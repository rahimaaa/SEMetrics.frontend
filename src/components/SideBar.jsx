...
function SideBar() {
  ...
  useEffect(() => {
    const fetchContributor = async () => {
      const contributors = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/account/repos/contributors/${repoName}`);
      setContributors(contributors.data);
      console.log(contributors)
    }
    const fetchRepo = async () => {
      const repo = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/account/repos/${repoName}`);
        ...
        <div className=" h-screen p-8">
          ...
          <div>
            {
              ...
              <button onClick={() => setShowAll(true)} className = "text-cyan-700">Show More...</button>: contributors.length > 3? <button onClick={() => setShowAll(false)} className = "text-cyan-700">Hide</button>: <div></div>
            }
          </div>
          ...
          <Link to={`/dashboard`}>
            <div className="flex mt-10 ml-2 space-x-3">
              ...
              <button type="button" className= "text-[#00BBE2] ">Go Back</button>
            </div>
          </Link>
